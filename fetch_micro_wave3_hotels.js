...
const microWave3Configs = [
  {
    ...
    queries: [
      { key: 'kusatsu_yubatake_view', query: '草津温泉 湯畑 旅館 露天風呂 群馬', label: '湯畑を望む特等席・温泉街中心部の老舗宿' },
      { key: 'kusatsu_sainokawara', query: '草津温泉 西の河原 ホテル 旅館 露天風呂', label: '西の河原公園周辺・大自然に抱かれる露天風呂宿' },
      { key: 'kusatsu_luxury_hideaway', query: '草津温泉 高級 旅館 露天風呂 客室 源泉', label: '草津の高台・静寂の隠れ家リゾート' }
    ],
    areaGuides: [
      {
  ...
  },
  {
    ...
    queries: [
      { key: 'takayama_old_town', query: '高山 古い町並 旅館 露天風呂 飛騨高山', label: 'さんまち通り・古い町並み徒歩圏内の老舗風情宿' },
      { key: 'takayama_hidagyu_gourmet', query: '飛騨高山 旅館 飛騨牛 露天風呂 部屋食', label: '特選A5飛騨牛会席と個室・部屋食が自慢の美食宿' },
      { key: 'takayama_station_walk', query: '高山駅 ホテル 旅館 温泉 徒歩 岐阜', label: '高山駅至近・白川郷観光の拠点に便利な宿' }
    ],
    areaGuides: [
      {
  ...
  },
  {
    ...
    queries: [
      { key: 'motohakone_jingu', query: '元箱根 箱根神社 ホテル 旅館 温泉 芦ノ湖', label: '箱根神社・平和の鳥居徒歩圏内の湖畔宿' },
      { key: 'ashinoko_lake_view', query: '芦ノ湖 ホテル 温泉 リゾート 富士山 レイクビュー', label: '客室から芦ノ湖と富士山を一望するリゾート' },
      { key: 'sengokuhara_susuki', query: '仙石原 すすき草原 ホテル 旅館 温泉 箱根', label: '仙石原・すすき草原とポーラ美術館周辺の高原宿' }
    ],
    areaGuides: [
      {
  ...
  },
  {
    ...
    queries: [
      { key: 'kamikochi_kappabashi', query: '上高地 河童橋 ホテル 旅館 穂高連峰 梓川', label: '河童橋畔・穂高連峰パノラマフロント宿' },
      { key: 'kamikochi_taishoike', query: '上高地 大正池 ホテル 温泉 長野', label: '大正池周辺・朝靄と立ち枯れの木の幻想宿' },
      { key: 'kamikochi_shirahone', query: '白骨温泉 旅館 露天風呂 秘湯 乳白色', label: '上高地玄関口・白骨温泉のにごり湯秘湯宿' }
    ],
    areaGuides: [
      {
  ...
  },
  {
    ...
    queries: [
      { key: 'miyajima_shrine_view', query: '宮島 嚴島神社 旅館 露天風呂 広島 温泉', label: '嚴島神社・大鳥居を望む宮島島内の老舗旅館' },
      { key: 'miyajima_omotesando', query: '宮島 表参道 ホテル 旅館 穴子 牡蠣', label: '表参道商店街・宮島桟橋至近の便利宿' },
      { key: 'miyajimaguchi_bay', query: '宮島口 ホテル 温泉 広島 オーシャンビュー', label: '宮島口・対岸から宮島を望むリゾートホテル' }
    ],
    areaGuides: [
      {
...
async function main() {
  console.log('=== Fetching direct hotel data from Rakuten Travel API for 5 Micro Tourist Hubs (Wave 3) ===');
  ...
  for (const theme of microWave3Configs) {
    console.log(`\n========================================`);
  ...
  console.log(`\nSuccessfully saved all Micro Tourist Hubs Wave 3 hotel data to: ${cachePath}`);
}
