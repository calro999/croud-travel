...
const microWave2Configs = [
  {
    ...
    queries: [
      { key: 'kamakura_station_walk', query: '鎌倉駅 ホテル 旅館 鶴岡八幡宮 徒歩', label: '鎌倉駅・小町通り・鶴岡八幡宮徒歩圏内の風情宿' },
      { key: 'shichirigahama_ocean', query: '七里ヶ浜 由比ヶ浜 ホテル オーシャンビュー 湘南', label: '七里ヶ浜・由比ヶ浜の海と富士山を望む絶景宿' },
      { key: 'enoshima_bayside', query: '江の島 ホテル 温泉 旅館 海鮮', label: '江の島島内＆片瀬海岸の展望温泉リゾート' }
    ],
    areaGuides: [
      {
  ...
  },
  {
    ...
    queries: [
      { key: 'kinosaki_river_walk', query: '城崎温泉 柳並木 旅館 露天風呂 兵庫', label: '大谿川・柳並木沿いの風情ある老舗旅館' },
      { key: 'kinosaki_crab_luxury', query: '城崎温泉 松葉ガニ 蟹 料理 旅館 高級', label: 'タグ付き松葉ガニ会席が自慢の名門宿' },
      { key: 'kinosaki_station_sotoyu', query: '城崎温泉駅 旅館 ホテル 外湯めぐり 徒歩', label: '城崎温泉駅至近・外湯めぐりに便利な宿' }
    ],
    areaGuides: [
      {
  ...
  },
  {
    ...
    queries: [
      { key: 'kyukaruizawa_classic', query: '旧軽井沢 ホテル 旅館 クラシック 雲場池', label: '旧軽井沢銀座・雲場池周辺のクラシック宿' },
      { key: 'nakakaruizawa_hoshino', query: '中軽井沢 ハルニレテラス ホテル 温泉 リゾート', label: '中軽井沢・星野エリア（ハルニレテラス）至近宿' },
      { key: 'karuizawa_forest_villa', query: '軽井沢 ヴィラ コテージ 温泉 暖炉', label: '森の中に佇むプライベートヴィラ＆温泉宿' }
    ],
    areaGuides: [
      {
  ...
  },
  {
    ...
    queries: [
      { key: 'wakura_bay_luxury', query: '和倉温泉 高級 旅館 露天風呂 能登 石川', label: '七尾湾を一望する名門オーシャンビュー旅館' },
      { key: 'wakura_seafood_gourmet', query: '和倉温泉 能登牛 のどぐろ 寒ブリ 旅館', label: '能登牛・寒ブリ・のどぐろ会席が自慢の美食宿' },
      { key: 'nanao_bay_resort', query: '七尾 和倉温泉 ホテル 温泉 露天風呂', label: '七尾湾沿いのリゾートホテル＆温泉宿' }
    ],
    areaGuides: [
      {
  ...
  },
  {
    ...
    queries: [
      { key: 'kinrin_lake_walk', query: '由布院 金鱗湖 旅館 露天風呂 温泉 湯布院', label: '金鱗湖周辺・朝霧を望む静寂の隠れ家宿' },
      { key: 'yunotsubo_street', query: '湯の坪街道 由布院 ホテル 旅館 湯布院 徒歩', label: '湯の坪街道・由布院駅徒歩圏内の快適宿' },
      { key: 'yufudake_luxury_villa', query: '由布院 離れ 露天風呂 由布岳 旅館 高級', label: '由布岳パノラマを望む全室離れ高級旅館' }
    ],
    areaGuides: [
      {
...
async function main() {
  console.log('=== Fetching direct hotel data from Rakuten Travel API for 5 Micro Tourist Hubs (Wave 2) ===');
  ...
  for (const theme of microWave2Configs) {
    console.log(`\n========================================`);
  ...
  console.log(`\nSuccessfully saved all Micro Tourist Hubs Wave 2 hotel data to: ${cachePath}`);
}
