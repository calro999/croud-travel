...
const articles = JSON.parse(fs.readFileSync(configPath, 'utf8'));

articles.forEach(article => {
  const dirPath = path.join(__dirname, 'src', 'app', article.slug);
...
export const metadata: Metadata = {
  title: "${article.title} ｜ 日本全国・旅宿クラウド",
  description: "${article.metaDesc.replace(/"/g, '\\"')}",
  keywords: ["${article.slug}", "高速バス", "バスツアー", "新幹線 比較", "安く行く方法", "1泊2日 モデルコース", "宿泊予約", "楽天トラベル"],
};
...
function loadHotels() {
  try {
    ...
    if (fs.existsSync(filePath)) {
      ...
      return data["${article.slug}"]?.hotels || [];
    }
  } catch (e) {
    console.error("Failed to load hotels for ${article.slug}", e);
  }
  return [];
}
...
export default function HighwayBusArticlePage() {
  ...
  const tableData = ${JSON.stringify(article.tableData, null, 2)};
  const busDetails = ${JSON.stringify(article.busDetails, null, 2)};
  const itinerary = ${JSON.stringify(article.itinerary, null, 2)};
...
`;
  ...
  fs.writeFileSync(path.join(dirPath, 'page.tsx'), pageTsxContent, 'utf8');
});
console.log('全30記事に楽天バスツアー・観光列車クロスセル導線を実装完了');
