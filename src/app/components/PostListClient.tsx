      ...
      <section className="bg-white border border-emerald-900/5 rounded-2xl p-5 md:p-6 shadow-sm space-y-5">
        ...
        <div className={`grid grid-cols-1 md:grid-cols-4 gap-4 ${isFilterOpen ? "block" : "hidden md:grid"}`}>
          ...
          <div className="space-y-1.5">
            ...
            <select
              value={selectedArea}
              onChange={(e) => { setSelectedArea(e.target.value); setSelectedPref("すべて"); }}
        ...
        {hasActiveFilters && (
          <div className="flex items-center justify-between text-xs pt-4 border-t border-emerald-950/5">
            ...
            <button
              onClick={() => { setSearchQuery(""); setSelectedArea("すべて"); setSelectedCategory("すべて"); setSelectedPref("すべて"); }}
            ...
            >
              <div className="aspect-[16/10] relative overflow-hidden bg-emerald-50 flex items-center justify-center border-b border-emerald-950/5">
                ...
                <div className="absolute top-4 left-4 flex gap-1.5">
                  <span className="text-[9px] font-extrabold bg-teal-800 text-white px-3 py-1 rounded-full shadow-sm">{post.area}</span>
                  <span className="text-[9px] font-extrabold bg-amber-600 text-white px-3 py-1 rounded-full shadow-sm">{post.prefecture}</span>
                </div>
                {post.price && (
              ...
              <div className="p-6 flex-grow flex flex-col justify-between space-y-4">
                <div className="space-y-3">
                  <div className="flex items-center justify-between text-[9px] font-bold text-emerald-950/40">
                    <span>{post.date}</span>
                    {post.rating && <span className="text-amber-600 font-extrabold flex items-center gap-0.5">⭐ {post.rating}</span>}
                  </div>
                  ...
                  <p className="text-xs text-emerald-950/60 leading-relaxed line-clamp-3 font-medium">
