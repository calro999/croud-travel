...
<div class="my-6 p-5 bg-white border border-emerald-950/20 rounded-2xl shadow-sm">
  ...
  <ul class="space-y-2 text-sm text-emerald-950/80 font-medium">
    ...
    <li><a href="#features" class="hover:text-emerald-600 hover:underline">2. 温泉・お部屋・設備の特徴</a></li>
...
<h2 id="features">温泉・お部屋・設備の特徴</h2>
...
def execute_all_remakes():
    ...
    for f in files:
        ...
        if not data.get('is_special_feature') and ('【おすすめ宿】' in title or '【旅ライター厳選】' in title or '観光】絶景とグルメを巡る' in title or '魅力と見どころガイド' in title):
            hotels_before, imgs_before, links_before = extract_preserve_info(data)
            
            remade_data = remake_single_hotel_post(data)
            
            hotels_after, imgs_after, links_after = extract_preserve_info(remade_data)
            
            # 保持情報の検証
            assert hotels_before.issubset(hotels_after), f"Hotel mismatch in {f}"
            assert imgs_before.issubset(imgs_after), f"Image mismatch in {f}"
            assert links_before.issubset(links_after), f"Link mismatch in {f}"
            
            with open(path, 'w', encoding='utf-8') as fp:
                json.dump(remade_data, fp, ensure_ascii=False, indent=2)
            
            modified_files.append(f)
            
    print(f"Successfully remade {len(modified_files)} posts.")
