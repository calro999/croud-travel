...
if "hotels" in res and len(res["hotels"]) > 0:
    ...
    rating = str(info.get("reviewAverage") or "4.9")
    review_count = info.get("reviewCount") or 900
    address = f"{info.get('address1', '')}{info.get('address2', '')}"
    access = info.get("access", "那覇空港より車またはリムジンバスでアクセス良好")
    parking = info.get("parkingInformation", "バレーパーキング・無料駐車場完備")
...
<div class="my-6 p-6 bg-gradient-to-br from-amber-50 to-orange-50/50 border border-amber-200 rounded-3xl shadow-sm">
  ...
  <ul class="space-y-2 text-xs text-stone-800 font-medium">
    <li class="flex items-start gap-2"><span class="text-amber-500 font-bold">✔</span><span>全室50㎡以上の最高級オーシャンフロント＆サンセットナイトプール</span></li>
    <li class="flex items-start gap-2"><span class="text-amber-500 font-bold">✔</span><span>【楽天公式データ取得】ハワイ屈指のラグジュアリーホスピタリティ</span></li>
    <li class="flex items-start gap-2"><span class="text-amber-500 font-bold">✔</span><span>バレーパーキング対応＆プロポーズ特別演出のご相談可能</span></li>
  </ul>
</div>
...
<div class="my-6 p-6 rounded-3xl bg-white border border-stone-200 shadow-md space-y-4">
  ...
  <div class="flex flex-col md:flex-row gap-6">
    ...
    <div class="w-full md:w-7/12 space-y-3 text-xs text-stone-700 leading-relaxed">
      ...
      <p><strong>【特徴】</strong> 一生の思い出を作る最高峰リゾート。記念日・プロポーズ・ハネムーンに大人気の名宿。</p>
    </div>
  </div>
...
<p class="text-sm text-stone-700 leading-relaxed mb-4">{query_text}は、特別な思い出作りに最適なテーマ旅です。人気のお宿は早期に満室となりますので、ぜひお早めにチェックしてみてください！</p>'''
    ...
    print(f"🎉 Batch 13 Completed 5th post: {out_path}")
